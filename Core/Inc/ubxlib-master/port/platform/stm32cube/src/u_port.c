...
// Also need these
#include "stm32h7xx_hal_rcc.h"  // RCC macros
#include "stm32h7xx_hal_pwr.h"  // PWR macros
#endif
...
static void systemClockConfig(void)
{
    ...
    RCC_OscInitStruct.PLL.PLLP = RCC_PLL2_DIVP;
                           ...
                           int32_t priority)
{
    ...
    if (pEntryPoint != NULL) {
        ...
        osThreadAttr_t threadAttr = {
            .name = "EntryPoint",
            .stack_size = stackSizeBytes,
            .priority = osPriorityNormal,
        };

        threadDef.attr.name = "EntryPoint";
        threadDef.pthread = (void (*) (void const *)) pEntryPoint;
        threadDef.attr.priority = priority;
        // Stack size is in words here, not bytes
        threadDef.attr.stack_size = stackSizeBytes >> 2;
