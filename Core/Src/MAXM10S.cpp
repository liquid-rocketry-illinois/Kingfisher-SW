...
uint16_t MAXM10S::readGPS(uint16_t numBytes, uint8_t *buffer) {
    ...
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(_hi2c, I2C_ADDRESS, REG_DATA_STREAM, I2C_MEMADD_SIZE_8BIT, buffer, numBytes, 1000);
        ...
        if (readGPS(bytesWaiting, gpsBuffer) > 0) {
            for (int i = 0; i < bytesWaiting; i++) {
                if (gpsBuffer[i] != 0xff && gpsBuffer[i] != 0x00) {
                    if (gpsParser.encode(gpsBuffer[i])) {
...
MAXM10S::gpsData MAXM10S::getData() {
