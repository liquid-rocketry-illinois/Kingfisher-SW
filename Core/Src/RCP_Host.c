...
static float toFloat(const uint8_t* start) { return ((float*) start)[0]; }
...
int RCP_isOpen(void) { return callbacks != NULL; }
...
int RCP_poll(void) {
    ...
    case RCP_DEVCLASS_POWERMON: {
        struct RCP_TwoFloat d = {.devclass = buffer[1], .timestamp = timestamp, .ID = ID};
...
int RCP_sendHeartbeat(void) { return RCP__sendTestUpdate(RCP_HEARTBEATS_CONTROL, 0x0F); }

int RCP_startTest(uint8_t testnum) { return RCP__sendTestUpdate(RCP_TEST_START, testnum); }

int RCP_stopTest(void) { return RCP__sendTestUpdate(RCP_TEST_STOP, 0); }

int RCP_pauseUnpauseTest(void) { return RCP__sendTestUpdate(RCP_TEST_PAUSE, 0); }

int RCP_deviceReset(void) { return RCP__sendTestUpdate(RCP_DEVICE_RESET, 0); }

int RCP_deviceTimeReset(void) { return RCP__sendTestUpdate(RCP_DEVICE_RESET_TIME, 0); }
...
int RCP_requestTestState(void) { return RCP__sendTestUpdate(RCP_TEST_QUERY, 0); }
